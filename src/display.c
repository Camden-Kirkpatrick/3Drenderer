...
// uint32_t colors[NUM_COLORS] = {
//     RED, GREEN, BLUE, DARK_BLUE, LIGHT_BLUE, YELLOW, MAGENTA, CYAN,
//     WHITE, BLACK, GRAY, DARK_GRAY, LIGHT_GRAY, ORANGE, PURPLE, PINK, BROWN,
// };
...
//     // INTERNAL render size (backbuffer/texture). If none provided, default to 1280x720.
//     if (req_w > 0 && req_h > 0) {
//         w->width  = req_w;
//         w->height = req_h;
//     } else {
...
//     // Create renderer (accelerated + VSYNC to avoid tearing)
...
//     // Queue the texture to be drawn on the screen (but not shown yet)
//     // (We also upscale to fill the fullscreen window — DISPLAY size.)
//     SDL_Rect dst = { 0, 0, w->screen_w, w->screen_h };
//     SDL_RenderCopy(w->renderer, w->color_buffer_texture, NULL, &dst);
// }
...
//     if (allocated < num_rects)
//     {
//         free(inited); free(rx); free(ry); free(rvx); free(rvy);
//         free(rcolor); free(size); free(ax); free(ay);
...
//             if (avg_speed < 5)       rcolor[i] = DARK_BLUE;
//             else if (avg_speed < 10) rcolor[i] = BLUE;
//             else if (avg_speed < 15) rcolor[i] = LIGHT_BLUE;
//             else if (avg_speed < 20) rcolor[i] = CYAN;
//             else                     rcolor[i] = GREEN;
...
//             rx[i] = 0; rvx[i] = -rvx[i];
//         } else if (rx[i] + size[i] >= w->width) {
//             rx[i] = w->width - size[i]; rvx[i] = -rvx[i];
//         }

//         if (ry[i] <= 0) {
//             ry[i] = 0; rvy[i] = -rvy[i];
//         } else if (ry[i] + size[i] >= w->height) {
//             ry[i] = w->height - size[i]; rvy[i] = -rvy[i];
//         }
...
// {
//     for (int i = 0; i < w->width * w->height; i++) {
//         w->z_buffer[i] = 0.0f; // using 1/w depth, keep larger => clear to 0.0
//     }
// }
...
// void cleanup_rectangles(void)
// {
//     free(inited); inited = NULL;
//     free(size);   size = NULL;
//     free(rx);     rx = NULL;
//     free(ry);     ry = NULL;
//     free(rvx);    rvx = NULL;
//     free(rvy);    rvy = NULL;
//     free(rcolor); rcolor = NULL;
//     free(ax);     ax = NULL;
//     free(ay);     ay = NULL;
//     allocated = 0;
// }
